...
<button type="button" onclick="deleteCodeBlock('|] <> sw pid <> [r|');" value="delete" id="search-button">
   ...
   <button type="button" onclick="addScoreCodeBlock('|] <> sw pid <> [r|');" value="addscore" id="search-button">
    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 512 512">
    <title></title>
    <g id="icomoon-ignore">
    </g>
   ...
   <button type="button" onclick="subtractScoreCodeBlock('|] <> sw pid <> [r|');" value="subtractscore" id="search-button">
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 512 512">
<title></title>
<g id="icomoon-ignore">
</g>
...
foldListList2 stylish allBlock = L.foldr(+)[]  zhtml3
               where
                ...
                -- TODO: Add different sortings flag from user input
...
{-|
  === Html table contains pdf file gallery
-}
...
responseGenePDFHtml conn = responseStream                                                
              ...
              pa <- getEnv "www" >>= \x -> return $ x </> "pdf/"  -- TEST ONLY
...
commandService conn ref req response = do
  ...
  if | cmd == cmdAlignment   -> do
     ...
     | cmd == cmdAlignmentStr -> do
       ...
       -- alignmentStr -p kkk       <-- pipe as stdin, kk is dummy  var. Not Used for Now
       ...
       then do
           ...
           logFileG ["ERROR: exitFailure:"++ cmdAlignmentStr]
     ...
     | cmd == cmdCommentCode -> do
       ...
       then do
           ...
           logFileG ["ERROR: exitFailure:" ++ cmdCommentCode]
           let upcodeblock' = updateRetcmd "CommentCode.sh.sh -p =" upcodeblock
           response $ responseJSON upcodeblock'
...
deleteCodeBlock conn ref req response = do
  ...
  let codeJson = case may of 
        (Just x) -> x 
        _        -> UpdateCodeBlock{pid = 0, newcode="no code deleteCodeBlock", begt=0, endt=0} 
  fw "deleteCodeBlock"
  pre codeJson
...
{-| 
    ...
    @
-} 
readDatabaseCodeBlock3::Connection -> IO [([String], ([String], Integer, Integer, Integer))]
readDatabaseCodeBlock3 conn = do
                      -- read CodeBlock table => [CodeBlock]::[Text]
                      mycode <- query_ conn "SELECT id, header, codeblock, addedtime, score from CodeBlock" :: IO [CodeBlock]
...
insertDatabaseNewCodeTable conn pid ucode = do
  ...
  -- executeMany conn Query{fromQuery = s2Text "INSERT INTO CodeBlock (header, codeblock) VALUES(?, ?)"} ([(header', ucode)]::[(TS.Text, TS.Text)])
